...
#    I_dp = bem.assembly.InverseSparseDiscreteBoundaryOperator(SparseDiscreteBoundaryOperator(csc_matrix(identity_dual_primal(vertices,verticesb, dofs,dofsb,triangles, 1, kappa , "P0d", "P1" ))))
    ...
    S_none2 = bem.operators.boundary.helmholtz.single_layer(P0b, P0b, P0b,kappa).weak_form()
    S_none = bem.operators.boundary.helmholtz.single_layer(P1, P1, P1,kappa).weak_form()                                                                                            
    S_0d = bem.operators.boundary.helmholtz.single_layer(P0d, P0d, P0d, kappa).weak_form()
    HP_none = bem.operators.boundary.helmholtz.hypersingular(P1, P0, P1, kappa).weak_form()
...
#    S_0dp = SparseDiscreteBoundaryOperator(csc_matrix(single_layer_primal(verticesb, trianglesb, dofsb, 1, kappa , "P0", "P0")))
    S_0dp = P0op*S_none2*P0op.T
#    end_time = time.time()                                                                                    
#    T2.append(end_time-start_time) 

#    start_time = time.time()                                                                                  
    H=hmat(vertices, verticesb, dofs, dofsb, rhs_none, 3, n, eps, eta, "P0d", "P0d", kappa,1)                    
#    end_time = time.time()                                                                                    
    plt.imshow(H.real- as_matrix(S_0d).real)                                                                                        
    plt.show()
...
#    S_0d2 = SparseDiscreteBoundaryOperator(csc_matrix(np.diag(np.diag(as_matrix(S_0dp)))+S_0d2))
#    end_time = time.time()
#    T2.append(end_time-start_time)

#    start_time = time.time()
#    S_none2 = SparseDiscreteBoundaryOperator(csc_matrix(single_layer_primal( vertices,triangles, dofs, 2, kappa , "P1", "P1")))
#    end_time = time.time()
#    T2.append(end_time-start_time)
